...
void generateFood(GameState& game_state) {
    ...
    printf("New food generated at (%d,%d) after %d attempts\n", 
           game_state.food.x, game_state.food.y, attempts);
}
...
void initializeGame(GameState& game_state) {
    ...
    printf("Snake initialized at center (%d,%d), length: %d, direction: RIGHT\n", 
           start_x, start_y, INITIAL_SNAKE_LENGTH);
    ...
    printf("Game state reset - Score: 0, Game over: false, Paused: false\n");
...
bool moveSnake(GameState& game_state) {
    ...
    if (new_head.x <= 0 || new_head.x >= GRID_WIDTH - 1 ||
        ...
        printf("GAME OVER: Boundary collision at (%d,%d)\n", new_head.x, new_head.y);
        return false; // 游戏结束
    }
    ...
    for (uint16_t i = 0; i < game_state.snake.length; i++) {
        if (game_state.snake.segments[i].x == new_head.x &&
            ...
            printf("GAME OVER: Self collision at (%d,%d) with segment %d\n", new_head.x, new_head.y, i);
            return false; // 游戏结束
        }
    }
    ...
    if (ate_food) {
        printf("FOOD EATEN! Snake length: %d -> %d, Score: %d -> %d\n", 
               game_state.snake.length, game_state.snake.length + 1,
...
int main() {
    stdio_init_all();
    printf("=== Snake Game for ILI9488 - Starting ===\n");
    printf("Screen: %dx%d, Grid: %dx%d\n", SCREEN_WIDTH, SCREEN_HEIGHT, GRID_WIDTH, GRID_HEIGHT);
    printf("Initial snake length: %d, Max length: %d\n", INITIAL_SNAKE_LENGTH, MAX_SNAKE_LENGTH);
    printf("Game speed: %dms\n", GAME_SPEED_MS);
    ...
    // 设置为竖屏模式
    printf("Setting LCD rotation to Portrait_180\n");
    ...
    if (!joystick.begin(JOYSTICK_GET_I2C_CONFIG())) {
        printf("ERROR: Joystick initialization failed!\n");
        return -1;
    }
    
    printf("Joystick initialization successful\n");
    printf("=== All hardware initialized successfully ===\n");
    ...
    // 等待开始
    printf("Waiting for MID button press to start...\n");
    bool started = false;
    while (!started) {
        if (joystick.get_button_value() == 0) {
            started = true;
            printf("MID button pressed - starting game\n");
            sleep_ms(200);  // 防抖延迟
        }
    ...
    printf("Game initialized - Snake at (%d,%d), Food at (%d,%d), Score: %d\n", 
           game_state.snake.segments[0].x, game_state.snake.segments[0].y,
           game_state.food.x, game_state.food.y, game_state.score);
    
    // 绘制初始游戏画面
    printf("Drawing initial game screen...\n");
    lcd_driver.fillScreenRGB666(BG_COLOR);
    ...
    while (true) {
        ...
        if (mid_pressed && !last_mid_pressed) {
            ...
            if (game_state.game_over || game_state.waiting_to_restart) {
                ...
                printf("Game restarted - Snake at (%d,%d), Food at (%d,%d)\n", 
                       game_state.snake.segments[0].x, game_state.snake.segments[0].y,
        ...
        if (game_state.game_over) {
            ...
            if (elapsed_time >= 5000) {
                // 5秒后进入等待重新开始状态
                printf("Countdown finished - waiting for restart\n");
                game_state.game_over = false;
        ...
        if (!mid_pressed && is_nil_time(last_red_time)) {
        ...
        if (current_time - last_move_time >= GAME_SPEED_MS) {
            ...
            if (!moveSnake(game_state)) {
                // 游戏结束
                printf("GAME OVER! Final score: %d, Snake length: %d\n", 
                       game_state.score, game_state.snake.length);
